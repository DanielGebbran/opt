...
#        MyHouse.HouseX.model.c1[i] = 10
    ...
    port = 5000; host = '172.16.13.73'; server = (host, port)
    s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    try:
        s.bind((host, 5002))
        print("Main port used.")
    except:
        s.bind((host, 5012))
        print("Alternative port used.")
    ...
    #        message = input("-> Type message: ")
            ...
            print ("No data received within a minute. Sleeping for one minute now...")
            #break
            time.sleep( 60 )
        except:
